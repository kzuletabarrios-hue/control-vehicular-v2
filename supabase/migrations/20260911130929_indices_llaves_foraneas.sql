-- NOTA (Jorge, actualizado 2026-09-14): 8 de los índices de este
-- archivo son sobre columnas que no tenían ALTER TABLE versionado en
-- git -- drift detectado el 2026-09-14:
--   - idx_archivos_citas_subido_por, idx_citas_programadas_archivo_id,
--     idx_citas_programadas_proveedor_id, idx_configuracion_updated_por
--     (tablas huérfanas, ver 00000000000000_LEAME_metodologia_y_hallazgos.sql
--     sección 2a) -- YA RESUELTO por
--     20260804100000_schema_tablas_huerfanas_drift.sql.
--   - idx_control_acceso_creado_por_fk, idx_flota_propia_creado_por_fk,
--     idx_proveedores_creado_por_fk, idx_visitantes_creado_por_fk
--     (columna `creado_por_fk` huérfana en las 4 tablas, hallazgo de
--     María corriendo las 46 migraciones contra una base vacía real,
--     2026-09-14) -- YA RESUELTO por
--     20260911130900_creado_por_fk_columnas_huerfanas.sql (fecha
--     inmediatamente anterior a este archivo).
-- Ambos archivos de fix quedan antes de este en la secuencia -- no
-- hace falta ninguna acción manual. En producción real todo este
-- archivo sigue siendo no-op seguro (todas las columnas/tablas ya
-- existían ahí desde antes).
-- ================================================================
